...
import { CustomTag, EmptyLine, RenderableContent } from '../templating/types';
...
export function extractCustomTags(type: DocCommentsAware): CustomTag[] {
  return (
    type.docComment?.annotations
      .filter((currentAnnotation) => !baseTags.includes(currentAnnotation.name.toLowerCase()))
      .map<CustomTag>((currentAnnotation) => ({
        name: currentAnnotation.name,
        value: documentationLinesToRenderableContent(currentAnnotation.bodyLines)!,
      })) ?? []
  );
}
